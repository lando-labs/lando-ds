 ...
 * Shared by `scripts/emit-tokens.mjs` (hex -> oklch() at emit time) and
 * `src/tokens/oklch-roundtrip.test.ts` (the perceptual-equivalence proof).
 *
 ...
 * (asserted in the round-trip test), which is what makes "the Ocean theme is
 * perceptually unchanged" a provable claim rather than an eyeball.
 *
 ...
 *
 * Published twin: `src/tokens/oklch.ts` is the consumer-facing TS port of this
 * file; `src/tokens/oklch-parity.test.ts` asserts the two never drift.
 */
...
export function rgbToHex({ r, g, b }) {
  const f = (n) => Math.round(n).toString(16).padStart(2, '0').toUpperCase()
  return `#${f(r)}${f(g)}${f(b)}`
}
 ...
 * @returns {{ L:number, C:number, H:number }} L,C in 0..~1; H in degrees 0..360.
 ...
 * @returns {{ r:number, g:number, b:number, inGamut:boolean }} channels 0..255
 *   (clamped); `inGamut` is false if any linear channel fell outside [0,1]
 *   beyond a tiny float epsilon before clamping.
 */
export function oklchToSrgb(L, C, H) {
  ...
  const clamp = (c) => Math.max(0, Math.min(255, linearToSrgbChannel(c)))
...
export const WHITE_OKLAB = oklabFromHex('#FFFFFF')
/** Pre-computed OKLab of pure black (`#000000`) — the darken endpoint. */
export const BLACK_OKLAB = oklabFromHex('#000000')
...
export function oklabDeltaE(o1, o2) {
