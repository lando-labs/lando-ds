/**
 * Stack Component
 *
 * Vertical flex layout primitive with consistent spacing.
 * Provides clean, declarative vertical layouts using design tokens.
 */

import React from 'react'
import styles from './Stack.module.css'

/** Keyword (token-backed) gap sizes that emit a CSS Module class. */
export type StackGapKeyword = 'none' | 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl'

const STACK_GAP_KEYWORDS: ReadonlySet<string> = new Set([
  'none',
  'xs',
  'sm',
  'md',
  'lg',
  'xl',
  '2xl',
])

export interface StackProps
  extends Omit<React.HTMLAttributes<HTMLElement>, 'children'> {
  /**
   * Spacing between children. Accepts EITHER a token keyword (preferred,
   * stays on the design-token rhythm) or a raw CSS-length value:
   *
   * - `'none' | 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl'` → token class
   * - `number` → emitted verbatim as `<n>px` inline style
   * - `string` (e.g. `'0.5rem'`, `'var(--spacing-3)'`) → emitted verbatim
   *
   * Numeric/string escape hatch added in #374 so consumers can size flex
   * gaps off the design-token grid (e.g. for label/control rhythm) without
   * losing the keyword shortcuts.
   *
   * @default 'md'
   */
  gap?: StackGapKeyword | (string & {}) | number

  /**
   * Horizontal alignment of children
   * @default 'stretch'
   */
  align?: 'start' | 'center' | 'end' | 'stretch'

  /**
   * Vertical alignment/distribution of children
   * @default 'start'
   */
  justify?: 'start' | 'center' | 'end' | 'between' | 'around' | 'evenly'

  /**
   * Allow children to wrap to next line
   * @default false
   */
  wrap?: boolean

  /**
   * Expand to full width of parent container
   * @default false
   */
  fullWidth?: boolean

  /**
   * Grow to fill remaining space when this Stack is itself a flex item of
   * a parent flex container (#374).
   *
   * - `true` → `flex: 1 1 auto`
   * - `<number>` → `flex: <n> 1 auto`
   */
  grow?: boolean | number

  /**
   * HTML element to render as
   * @default 'div'
   */
  as?: 'div' | 'section' | 'article' | 'aside' | 'nav'

  /**
   * Child elements to render in vertical stack
   */
  children: React.ReactNode

  /**
   * Additional CSS class names
   */
  className?: string

  /**
   * Inline styles
   */
  style?: React.CSSProperties
}

/**
 * Stack - Vertical flex layout primitive
 *
 * @example
 * ```tsx
 * <Stack gap="md" align="center">
 *   <Heading level={1}>Welcome</Heading>
 *   <Text>This is a description</Text>
 *   <Button>Get Started</Button>
 * </Stack>
 * ```
 */
export const Stack = React.forwardRef<HTMLDivElement, StackProps>(
  (
    {
      gap = 'md',
      align = 'stretch',
      justify = 'start',
      wrap = false,
      fullWidth = false,
      grow,
      as: Component = 'div',
      children,
      className = '',
      style,
      ...rest
    },
    ref
  ) => {
    // Resolve `gap`. Keyword → CSS Module class (preferred path, stays on
    // the design-token rhythm). Number → `<n>px` inline. String → verbatim.
    const isKeywordGap =
      typeof gap === 'string' && STACK_GAP_KEYWORDS.has(gap)
    const gapClass = isKeywordGap ? styles[`gap-${gap}`] : ''

    let inlineGap: string | undefined
    if (!isKeywordGap) {
      if (typeof gap === 'number') {
        inlineGap = `${gap}px`
      } else if (typeof gap === 'string') {
        inlineGap = gap
      }
    }

    // Resolve `grow`. See Box for the same shape.
    let flexValue: string | undefined
    if (grow === true) {
      flexValue = '1 1 auto'
    } else if (typeof grow === 'number' && grow > 0 && Number.isFinite(grow)) {
      flexValue = `${grow} 1 auto`
    }

    const classNames = [
      styles.stack,
      gapClass,
      styles[`align-${align}`],
      styles[`justify-${justify}`],
      wrap && styles.wrap,
      fullWidth && styles.fullWidth,
      className,
    ]
      .filter(Boolean)
      .join(' ')

    // Merge inline styles. User `style` first; computed (gap + flex) second
    // so prop-driven layout wins on conflict — mirrors the Grid policy.
    const mergedStyle: React.CSSProperties = {
      ...style,
      ...(inlineGap !== undefined ? { gap: inlineGap } : {}),
      ...(flexValue !== undefined ? { flex: flexValue } : {}),
    }

    return (
      <Component
        ref={ref as React.Ref<HTMLDivElement>}
        className={classNames}
        style={mergedStyle}
        {...rest}
      >
        {children}
      </Component>
    )
  }
)

Stack.displayName = 'Stack'
