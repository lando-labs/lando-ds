...
- [#98](https://github.com/lando-labs/lando-ds/pull/98) [`09c9b1d`](https://github.com/lando-labs/lando-ds/commit/09c9b1ddf4d2254bf3f3bd9984df680b3341c42f) Thanks [@Lando8604](https://github.com/Lando8604)! - `Avatar` gradient-initials now clear WCAG AA (SC 1.4.3, 4.5:1) for white text at every size, in both light and dark mode, and across the brand-neutral default and the `lando` theme preset ([#83](https://github.com/lando-labs/lando-ds/issues/83)).

  Measured against every color stop of every gradient variant (`.gradient`, `.gradient-0`…`.gradient-6`, light + `[data-theme='dark']`), the `-light`/`-base` rungs of both `--color-primary-*` and `--color-secondary-*` fail everywhere — as low as **1.75:1** (`--color-secondary-light`, `lando` preset, light mode) — and `--color-secondary-medium` additionally fails under the `lando` preset (3.45:1). The specific pairing the issue reported, brand-neutral default dark-mode `--color-primary-base`, measured **3.23:1**. `--color-primary-base` itself cannot move: its dark value is CI-locked by the [#73](https://github.com/lando-labs/lando-ds/issues/73) fix (Button outline-label AA on an elevated surface), so this had to be solved at the Avatar layer, not the shared token.

  **Three directions were weighed** (see the `[#83](https://github.com/lando-labs/lando-ds/issues/83)` comment block in `Avatar.module.css`):
  ...
  Closes [#83](https://github.com/lando-labs/lando-ds/issues/83).

- [#97](https://github.com/lando-labs/lando-ds/pull/97) [`8cccb39`](https://github.com/lando-labs/lando-ds/commit/8cccb39e0db2ac3954e95c522d4b72583937a456) Thanks [@Lando8604](https://github.com/Lando8604)! - **Combobox:** clicking the caret/chevron now opens the listbox ([#74](https://github.com/lando-labs/lando-ds/issues/74)).
...
- [#94](https://github.com/lando-labs/lando-ds/pull/94) [`b94af6f`](https://github.com/lando-labs/lando-ds/commit/b94af6fb5ee2bf5216662df7443599f91dbf4ec5) Thanks [@Lando8604](https://github.com/Lando8604)! - **SegmentedControl:** fix the sliding selection indicator being mis-sized and mis-positioned on first paint until an interaction forced a re-measure ([#91](https://github.com/lando-labs/lando-ds/issues/91)).
...
- [#95](https://github.com/lando-labs/lando-ds/pull/95) [`fa57177`](https://github.com/lando-labs/lando-ds/commit/fa571778783d7193cef7021af0af6345f72feb61) Thanks [@Lando8604](https://github.com/Lando8604)! - `ThemeScope` now re-derives the mode-dependent BASE tokens (not just the [#11](https://github.com/lando-labs/lando-ds/issues/11) ramp/state formulas) for its own resolved mode, fixing nested-opposite-mode scoping ([#92](https://github.com/lando-labs/lando-ds/issues/92)) and DetailCard's illegible default field row in dark mode ([#93](https://github.com/lando-labs/lando-ds/issues/93)).

  Follow-on to [#11](https://github.com/lando-labs/lando-ds/issues/11) (PR [#86](https://github.com/lando-labs/lando-ds/issues/86)): that fix re-derived the tonal-ramp/interaction-state `color-mix()` FORMULAS on a `ThemeScope` wrapper. This issue is the other half — tokens whose value simply _differs_ by mode (`--color-surface-elevated`, `--color-text-primary`, `--shadow-md`, …) are declared once at `:root` and once more inside `[data-theme="dark"]` in `src/styles/tokens.css`. Neither real CSS rule ever matches a non-root `ThemeScope` wrapper, so a nested scope whose `mode` differs from the ambient page rendered these tokens with the ambient page's mode, not its own — even though the scope's own `data-theme` attribute (and therefore any `[data-theme='dark'] .foo`-gated component rule) was already correct.

  **Two compounding causes (both diagnosed in [#92](https://github.com/lando-labs/lando-ds/issues/92)):**
  ...
  2. ~43 components gate mode styling on `[data-theme='dark'] .foo` ANCESTOR selectors, which no CSS-only scope override can fully cancel — a real (but separate) architectural issue, deliberately **out of scope** for this fix (documented as follow-up work; see the PR description). This patch is (a) from the issue's own diagnosis: it's sufficient for the reported cases because most of those dark rules only swap _which_ token is read — if the token itself now resolves to the scope's mode, the rendered result is correct even when the ancestor-gated rule still matches.

  **The fix:**

  - New module `src/utils/scopedModeTokens.ts` — mechanically derived from `tokens.css`, mirroring two token shapes: (1) tokens whose literal value differs between `:root` and `[data-theme="dark"]` (excluding `--color-primary-base`, already owned end-to-end by `colorDerivation.ts`/[#11](https://github.com/lando-labs/lando-ds/issues/11) as a formula), and (2) tokens declared once at `:root` as a pure `var(--other)` indirection to one of those base tokens (`--color-surface-secondary`/`-tertiary`, `--color-bg-*`, `--color-editorial-*`, …) — the exact mechanism behind [#93](https://github.com/lando-labs/lando-ds/issues/93) (DetailCard's default `.field` row reads `--color-surface-tertiary`, never the base token directly).
  - `computeThemeAttrs`'s existing `deriveScopedTokens` flag (already opt-in, ThemeScope-only) now seeds `vars` with `getScopedModeTokenVars(mode)` alongside the [#11](https://github.com/lando-labs/lando-ds/issues/11) ramp formulas, at the same low precedence — an explicit preset/product-theme override still wins. The root `ThemeProvider`/`applyTheme` path is unchanged (`:root`'s real CSS already covers `document.documentElement` directly).
