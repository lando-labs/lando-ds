...
| `ThemeScope` | **Inherited `system`-mode `window.matchMedia` (#501)** | A `ThemeScope` with no `mode` prop under a `system`-mode `ThemeProvider` (no `initialMode`/`forcedTheme`) would otherwise hydration-mismatch: the provider's `theme` state seeds from `resolveTheme('system')`, which reads `window.matchMedia` inside a `useState` initializer — undefined on the server (→ `'light'`), but already resolved to the real OS preference on the client's very first hydration render. `ThemeScope.tsx` guards this case (placeholder render + a post-mount client-only correction); an explicit `mode` prop, or an inherited mode from a provider given `initialMode`/`forcedTheme`, is SSR-stable and unaffected. See the "Inherited-mode SSR guard (#501)" doc block at the top of `ThemeScope.tsx`. |
